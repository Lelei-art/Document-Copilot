...
@patch("app.retrieval.keywords._client")
def test_extract_fts_keywords_falls_back_when_llm_raises(
    mock_client: MagicMock,
) -> None:
    mock_client.return_value.chat.completions.parse.side_effect = RuntimeError(
        "api down"
    )
...
@patch("app.retrieval.keywords._client")
def test_extract_fts_keywords_falls_back_when_too_few_terms(
    mock_client: MagicMock,
) -> None:
    mock_client.return_value.chat.completions.parse.return_value = _mock_parse_response(
        ["Azure"]
...
@patch("app.retrieval.keywords._client")
def test_extract_fts_keywords_falls_back_when_parse_is_none(
    mock_client: MagicMock,
) -> None:
    message = MagicMock()
    message.parsed = None
