...
def test_map_chunk_record_extracts_section(sample_html_path: Path) -> None:
    ...
    assert (
        record.chunk_metadata["accession_number"] == FILING_METADATA["accession_number"]
    )
...
def test_chunk_document_adds_semantic_table_row_chunks(sample_html_path: Path) -> None:
    ...
    assert (
        "| iPhone | $209,586 | 4% | $201,183 | - | $200,583 |" in table_records[0].text
    )
    assert "iPhone | iPhone | iPhone" not in "\n".join(
        record.text for record in records
    )
    assert (
        table_records[0].chunk_metadata["table"]["title"]
